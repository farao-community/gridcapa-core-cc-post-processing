...
public final class NamingRules {
    ...
    // We must manually disable SONAR on lines below because a bug in the java:S3986 rule triggers a warning on the use of "Y" in the pattern even if it is escaped (placed between single quotes in order to be used as String and not to be interpreted by the formatter)
    // ZIP
    private static final DateTimeFormatter CGM_ZIP_FILENAME_FORMATTER = DateTimeFormatter.ofPattern("'22XCORESO------S_10V1001C--00236Y_CORE-FB-304_'yyyyMMdd'-F304-<version>.zip'").withZone(IntervalUtil.ZONE_ID); // NOSONAR
    private static final DateTimeFormatter CNE_ZIP_FILENAME_FORMATTER = DateTimeFormatter.ofPattern("'22XCORESO------S_10V1001C--00236Y_CORE-FB-B06A43-299_'yyyyMMdd'-F299-<version>.zip'").withZone(IntervalUtil.ZONE_ID); // NOSONAR
    private static final DateTimeFormatter LOGS_ZIP_FILENAME_FORMATTER = DateTimeFormatter.ofPattern("'22XCORESO------S_10V1001C--00236Y_CORE-FB-342_'yyyyMMdd'-F342-<version>.zip'").withZone(IntervalUtil.ZONE_ID); // NOSONAR
    ...
    // NON-ZIP
    private static final DateTimeFormatter CBCORA_FILENAME_FORMATTER = DateTimeFormatter.ofPattern("'22XCORESO------S_10V1001C--00236Y_CORE-FB-B06A01-303_'yyyyMMdd'-F303-<version>.xml'").withZone(IntervalUtil.ZONE_ID); // NOSONAR
